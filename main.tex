...
Additionally, we expand the notion of NCD~\cite{opitz2023gzip,weinreich2023parameter,nishida2011tweet,ncd,jiang2022less,jiang2022less} to kernels (Section~\ref{kernels}) in an attempt to use it to model more complex decision boundaries than those allowed by NCD as a distances alone.
...
Figure~\ref{fig:acc} shows the classifier performance across each dataset and string metric (including NCD, using various compressors). 
The top sub-figure groups the results by model type using colour, and the bottom figure groups the results by symmetrisation method.
From these results it is clear that KNN is the most consistently accurate (top of Figure~\ref{fig:acc}) across all of the metrics. 
...
Even if a model is trained entirely on private data and stored privately, the model itself can still be targeted by cross-site scripting attacks~\cite{xss}, though these can be prevented categorically using known, best practices~\cite{xss}. 
Due to the efficacy of NCD when used as a kernel method on even a small number of samples (Figure~\ref{fig:sample_size}) and run-times on the order of a few milliseconds (Figures~\ref{fig:times}), the proposed method can be used to train and deploy models entirely on a client device, thus reducing the attack surface to user-error. 
