...
\cm{
In an increasingly digital and connected world, user privacy and platform security are often orthogonal -- or even competing -- goals.
Balancing the privacy concerns of users with the security concerns of platform operators is of increasing importance to regulators and client-side neural network models (trained on user data in the cloud) have been proposed as a solution.
The problem with this approach is that neural networks have incredibly complex architectures with long run-times, require special hardware, are built using massive datasets, have opaque decision making processes, are trivial to circumvent, and can be used by an adversary to generate the malicious content that the model was designed to detect.
An alternative approach that uses compression algorithms to measure the distance between two objects has proven remarkably successful at classification tasks across a wide-variety of datasets---using only a small number of samples. 
Furthermore, the resulting model lends itself to a direct interpretation as similar inputs will be close to each other in the feature space.
The increased interpretability and small training set requirements make this method ideal for a private text classification system. 
}
In this paper, we present a client-side approach to malicious user detection that is as accurate as neural networks without revealing users' private data to the platform operator. 
In addition, we demonstrate the efficacy of this technique in the domain of computer security by testing many datasets like system-level computer process data, Twitter bot text, intrusion attacks, and denial of service attacks. 
We present methods for improving the model latency and evaluate this compression-based distance measure in the wider context of kernel metrics to show the broad efficacy  of these techniques in a wide variety of machine learning models.
\end{abstract}
