...
def flatten_directories(directories):
    ...
    for directory in directories:
        if not Path(directory).exists():
            logger.error(directory + ' does not exist.  Cannot flatten.')
            exit()
        else:
...
def main(args):
    ...
    if  directories is not None and isinstance(directories, str):
        directories = [directories]
    elif directories is not None and isinstance(directories, list):
        pass
